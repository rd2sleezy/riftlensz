"""Research-only exact R-012 finding-window capture + V.1 analysis.

Set RIFTLENS_V1_T4=1 to request a real R.10 capture. Without the flag this
script only resolves the finding and prints the planned window — it does
not auto-capture production findings and does not hardcode a .rofl path.
"""
...
def _load_findings(match_id: str) -> list[FindingStamp]:
    ...
    rows = con.execute(
        ...
        WHERE r.match_id = ? AND f.rule_id = ?
        ORDER BY f.t_ms
        """,
        (match_id, RULE_ID),
    ).fetchall()
...
def _gst_from_db(match_id: str, subject_pid: int) -> GameStateTimeline | None:
    ...
    provenance = Provenance(producer="v1-research-db", producer_version=1)
    facts: list[Fact] = []
    for row in kills:
...
async def _capture(plan_source_id: str, plan: PlannedCapture) -> tuple[int, Path | None]:
    rofl = _resolve_rofl()
    if rofl is None:
        print("FAIL: NA1-5617764200.rofl not found via env or Documents/Replays")
        return 2, None
    settings = get_settings()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="V.1 exact finding-window capture")
    parser.add_argument("--fps", type=float, default=DEFAULT_V1_SAMPLE_FPS)
    parser.add_argument("--json-out", type=Path, default=None)
    parser.add_argument("--analyze-only", action="store_true")
    args = parser.parse_args()
    findings = _load_findings(MATCH)
    finding = select_finding(
        findings, rule_id=RULE_ID, match_id=MATCH, prefer_t_ms=PREFERRED_T_MS
    )
    ...
    print("FINDING", finding.finding_id, finding.t_ms, stamp)
    ...
    enabled = os.environ.get("RIFTLENS_V1_T4") == "1"
    ...
    claim = StructuredClaim(
        rule_id=RULE_ID,
        t_ms=finding.t_ms,
        summary=finding.title or "Fought into unaccounted-for enemies",
    )
